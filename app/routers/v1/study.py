...
async def create_studies(request: study_schema.StudyCreate, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> study_schema.StudyShow:
...
async def update_study(study_id: int, request: study_schema.StudyUpdate, user: dict = Depends(get_current_user),study_Service: StudyService = Depends(get_study_service)) -> study_schema.StudyShow:
...
async def delete_study(study_id: int, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> bool:
...
async def upload_image(study_id: int, file: UploadFile = File(...), user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> study_schema.StudyShow:
    study = study_Service.show(study_id)
    if not study:
        raise HTTPException(status_code=404, detail=f"Study with id {study_id} not found")
...
async def archive_study(study_id: int, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> bool:
...
async def unarchive_study(study_id: int, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> bool:
    # check if user is doctor and is assigned to the study
    if user["role"] != "doctor":
...
async def assign_doctor(study_id: int, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> bool:
    # check if user is doctor and is assigned to the study
    if user["role"] != "doctor":
...
async def unassign_doctor(study_id: int, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> bool:
    # check if user is doctor and is assigned to the study
    if user["role"] != "doctor":
...
async def get_results(study_id: int, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> List[result_schema.ResultShow]:
    return study_Service.get_results(study_id)

@router.post("/{study_id}/results", dependencies=[Security(security)])
async def create_result(study_id: int, request: result_schema.ResultCreate, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> result_schema.ResultShow:
...
async def get_result(study_id: int, result_id: int, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> result_schema.ResultShow:
    return study_Service.get_result(study_id, result_id)

@router.put("/{study_id}/results/{result_id}", dependencies=[Security(security)])
async def update_result(study_id: int, result_id: int, request: result_schema.ResultUpdate, user: dict = Depends(get_current_user), study_Service: StudyService = Depends(get_study_service)) -> result_schema.ResultShow:
